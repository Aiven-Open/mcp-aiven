...
import { formatPgEditorQueryResult, RATE_LIMIT_MAX } from '../../src/tools/pg/query.js';
...
function createMockClient(opts: { postResponse?: unknown }): AivenClient {
...
describe('aiven_pg_read', () => {
  ...
  it('should reject queries when rate limit is exceeded', async () => {
    ...
    expect(client.post).toHaveBeenCalledTimes(RATE_LIMIT_MAX);
  });
  ...
  it('should allow queries again after rate limit window passes', async () => {
    ...
    let result = await tool.handler(params, { token: 'same-token' });
    expect(result.isError).toBe(true);

    vi.advanceTimersByTime(60001);

    result = await tool.handler(params, { token: 'same-token' });
    expect(result.isError).toBeUndefined();
